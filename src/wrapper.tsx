...
// @ts-ignore
import { RayTracingRenderer } from "./main";
...
const Raytracer = React.forwardRef(
  (
    { children, renderIndex = 1, samples = 64, ...props }: RaytracerProps,
    forwardRef: any
  ) => {
    const ref = React.useRef<THREE.Scene>(null!);
    const renderer = React.useRef<RayTracingRenderer>();
    const { scene, controls, camera, gl, size, viewport } = useThree();

    // Set up the renderer
    React.useEffect(() => {
      renderer.current = new RayTracingRenderer({
        // Fake a canvas to trick LGL into using the existing WebGL context
        canvas: {
          getContext: gl.getContext,
          getExtension: (gl as any).getExtension,
        },
    ...
    React.useEffect(() => {
      ...
      if (denoiseColorBlendFactor !== undefined)
        renderer.current.setDenoiseColorBlendFactor(denoiseColorBlendFactor);
      if (denoiseMomentBlendFactor !== undefined)
        renderer.current.setDenoiseMomentBlendFactor(denoiseMomentBlendFactor);
      if (denoiseColorFactor !== undefined)
        renderer.current.setDenoiseColorFactor(denoiseColorFactor);
      if (denoisePositionFactor !== undefined)
        renderer.current.setDenoisePositionFactor(denoisePositionFactor);
      update();
    }, [props]);
    ...
    React.useEffect(
      () => void renderer.current.buildScene(ref.current, camera).then(update),
      []
    );
    ...
    useFrame(() => {
      if (
        renderer.current &&
        renderer.current.pipeline &&
        renderer.current.getTotalSamples() < samples
      )
        renderer.current.render(ref.current, camera);
    }, renderIndex);
