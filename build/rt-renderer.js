(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["rt-renderer"] = {}, global.THREE));
  ...
  class RayTracingMaterial extends THREE$1.Material {
    constructor(parameters) {
      super();
      this.materialType = "RayTracingMaterial";
      this.isRayTracingMaterial = true;
      this.workflow = "Metalness";
      this.color = new THREE$1.Color(16777215);
      this.roughness = 0.5;
      this.metalness = 0;
      this.map = null;
      this.emissive = new THREE$1.Color(0);
      this.emissiveMap = null;
      this.normalMap = null;
      this.normalScale = new THREE$1.Vector2(1, 1);
      ...
      this.transmission = 0;
      this.atDistance = 1;
      this.extinction = new THREE$1.Color(16777215);
      this.anisotropic = 0;
      this.specularColor = new THREE$1.Color(16777215);
      this.glossiness = 1;
    ...
    copy(source) {
      ...
      this.color = new THREE$1.Color().copy(source.color);
      this.roughness = source.roughness;
      this.metalness = source.metalness;
      this.map = source.map;
      this.emissive = new THREE$1.Color().copy(source.emissive);
      this.emissiveMap = source.emissiveMap;
      this.normalMap = source.normalMap;
      this.normalScale = new THREE$1.Vector2().copy(source.normalScale);
      ...
      this.extinction = new THREE$1.Color().copy(source.extinction);
  ...
  function decomposeScene(scene, camera) {
      ...
      if (child.isMesh) {
        if (!child.geometry) {
          ...
          if (child.material.isMeshStandardMaterial) {
            child.material = new RayTracingMaterial().fromStandardMaterial(child.material);
          } else {
            child.material.isRayTracingMaterial || (child.material = new RayTracingMaterial().fromBasicMaterial(child.material));
          }
    ...
    if (meshLightsNum) {
      meshLights = mergeLightsFromScene(lights.map(light => {
        const meshLight = {};
        ...
        meshLight.p1 = new THREE$1.Vector3();
        meshLight.p2 = new THREE$1.Vector3();
        ...
        switch (light.type) {
          case "RectAreaLight":
            ...
            if (light.width && light.height) {
              const planeGeometry = new THREE$1.PlaneGeometry(light.width, light.height);
              const targetPosition = new THREE$1.Vector3();

              if (light.target) {
                targetPosition.copy(light.target);
              }

              const planeGeometryPosition = new THREE$1.Vector3().subVectors(light.position, targetPosition);
              const lookAtPosition = new THREE$1.Vector3().copy(planeGeometryPosition).negate();
              planeGeometry.lookAt(lookAtPosition);
              const positionAttributes = planeGeometry.attributes.position.array;
              const v1 = new THREE$1.Vector3(positionAttributes[0], positionAttributes[1], positionAttributes[2]).add(light.position); // Unused v2 vertex
              ...
              const v3 = new THREE$1.Vector3(positionAttributes[6], positionAttributes[7], positionAttributes[8]).add(light.position);
              const v4 = new THREE$1.Vector3(positionAttributes[9], positionAttributes[10], positionAttributes[11]).add(light.position);
              meshLight.position.copy(v3);
              meshLight.p1 = v4.sub(v3);
              meshLight.p2 = v1.sub(v3);
              meshLight.area = new THREE$1.Vector3().crossVectors(meshLight.p1, meshLight.p2).length();
            }
  ...
  function mergeMeshesToGeometry(meshes) {
    ...
    for (const mesh of meshes) {
      ...
      const geometry = mesh.geometry.isBufferGeometry ? cloneBufferGeometry(mesh.geometry, ["position", "normal", "uv"]) // BufferGeometry object
  ...
  function makeTexture(gl, params) {
    ...
    let target;
    let dataArray; // if data is a JS array but not a TypedArray, assume data is an array of images and create a GL Array Texture
    ...
    const {
      ...
      format,
      internalFormat
    } = getTextureFormat(gl, channels, storage, data, gammaCorrection);
    ...
    if (dataArray) {
      ...
      for (let i = 0; i < dataArray.length; i++) {
        ...
        gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, Array.isArray(flipY) ? flipY[i] : flipY), gl.texSubImage3D(target, 0, 0, 0, i, layerWidth, layerHeight, 1, format, type, dataArray[i]);
      }
    } else if (length > 1) {
  ...
  function getTextureFormat(gl, channels, storage, data, gammaCorrection) {
    let type;
    let internalFormat;
    const isByteArray = data instanceof Uint8Array || data instanceof HTMLImageElement || data instanceof HTMLCanvasElement || data instanceof ImageData || data instanceof ImageBitmap;
  ...
  function makeTileRender(gl) {
    ...
    let width = 0;
    let height = 0;
    let totalElapsedMs; // initial number of pixels per rendered tile
    ...
    function calcTileDimensions() {
      const aspectRatio = width / height; // quantize the width of the tile so that it evenly divides the entire window
    ...
    function updatePixelsPerTile() {
      ...
      const error = desiredMsPerTile - msPerTile; // tweak to find balance. higher = faster convergence, lower = less fluctuations to microstutters

      const strength = 5e3; // sqrt prevents massive fluctuations in pixelsPerTile for the occasional stutter

      pixelsPerTile += strength * Math.sign(error) * Math.sqrt(Math.abs(msPerTile));
  ...
  function makeRenderPass(gl, params) {
    ...
    return { ...makeRenderPassFromProgram(gl, program),
  ...
  function addDefines(defines) {
    ...
    for (const name in defines) {
      const value = defines[name]; // don't define falsy values such as false, 0, and ''.
  ...
  function makeFullscreenQuad(gl) {
    ...
    const vertexShader = makeVertexShader(gl, {
      vertex: vertex$1
    });
  ...
  function makeRenderSize(gl) {
    ...
    let renderWidth;
    let renderHeight;
    const scale = new THREE$1.Vector2(1, 1);
  ...
  function makeFramebuffer$1(gl, {
    ...
    function init() {
      ...
      for (let location in color) {
        ...
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + location, tex.target, tex.texture, 0), drawBuffers.push(gl.COLOR_ATTACHMENT0 + location);
      }
  ...
  function makeSVGFPass(gl, params) {
    ...
    function initFrameBuffer(width, height) {
      const makeSVGFBuffer = () => makeFramebuffer$1(gl, {
        color: {
          0: makeTexture(gl, {
            width,
            height,
  ...
  function makePrimitiveInfo(geometry) {
    const primitiveInfo = [];
    const indices = geometry.getIndex ? geometry.getIndex().array : geometry.index.array;
    const position = geometry.getAttribute ? geometry.getAttribute("position") : geometry.attributes.position;
    const materialMeshIndex = geometry.getAttribute ? geometry.getAttribute("materialMeshIndex") : geometry.attributes.materialMeshIndex;
    ...
    for (let d = 0; d < indices.length; d += 3) {
      ...
      const center = bounds.getCenter(new THREE$1.Vector3());
      const faceNormal = new THREE$1.Vector3().crossVectors(e1, e0).normalize();
      const materialIndex = materialMeshIndex.getX ? materialMeshIndex.getX(i0) : materialMeshIndex.array[i0 * materialMeshIndex.itemSize];
      const info = {
        bounds,
        center,
        indices: [i0, i1, i2],
  ...
  function recursiveBuild(primitiveInfo, start, end) {
    ...
    if (nPrimitives === 1) {
      ...
      let mid = Math.floor((start + end) / 2); // middle split method
  ...
  function generateBackgroundMapFromSceneBackground(background) {
    ...
    if (background.data.type === THREE__namespace.UnsignedByteType) {
      if (backgroundImage.data) {
        backgroundImage.data = rgbeToFloat(backgroundImage.data, background.intensity);
      } else {
        backgroundImage.data = clampedArrayFromImageData(background.data.image);
        backgroundImage.dataFormat = "byte";
      }
    } else if (background.data.type === THREE__namespace.FloatType && background.data.format === THREE__namespace.RGBAFormat) {
    ...
    } else if (background.data.type == THREE__namespace.HalfFloatType) {
      console.error("Please use 'new RGBELoader().setDataType(THREE.FloatType)' to load hdr env map. Half-Float type will loss of precision and have an impression of the effect.");
    } else if (background.data.type !== THREE__namespace.FloatType) {
      console.error(`No support environmentLight's data type: ${background.data.type.toString()}`);
    }
  ...
  */
  ...
  async function makeRayTracePass(gl, {
    bounces,
    // number of global illumination bounces
    decomposedScene,
    fullscreenQuad,
    materialBuffer,
    mergedMesh,
    ...
    function updateEnvLight(decomposedScene) {
      ...
      if (isTextureEnv) {
        var _environment$data;

        let envImage;

        if ((_environment$data = environment.data) != null && _environment$data.isTexture) {
        ...
        if (backgroundColor && backgroundColor.isColor) {
          renderPass.setUniform("backgroundColor", [backgroundColor.r, backgroundColor.g, backgroundColor.b]);
        } else {
  ...
  async function makeRenderPassFromScene({
    ...
    if ("function" == typeof (loadingCallback == null ? void 0 : loadingCallback.onProgress)) {
  ...
  function makeReprojectPass(e, params) {
    ...
    function draw(params) {
      ...
      renderPass.setTexture("previousMomentLengthVarianceTex", previousMomentLengthVariance);
  ...
  const toneMapFunctions = {
    [THREE__namespace.LinearToneMapping]: 0,
    [THREE__namespace.ACESFilmicToneMapping]: 1,
    [THREE__namespace.ReinhardToneMapping]: 2,
    [THREE__namespace.CineonToneMapping]: 3
  };
  function makeToneMapPass(gl, params) {
    ...
    const defaultLightScale = new THREE__namespace.Vector2(1, 1);
    ...
    function setSize(width, height) {
      frameBuffer = makeFramebuffer$1(gl, {
        color: {
          0: makeTexture(gl, {
            width,
            height,
...
`;
  ...
  function getUniformBlockInfo(gl, program, blockIndex) {
    ...
    for (let i = 0; i < indices.length; i++) {
      const {
        ...
        type,
        size
      } = gl.getActiveUniform(program, indices[i]);
      uniforms[name] = {
        type,
        size,
        offset: offset[i],
  ...
  function setData(dataView, setter, size, offset, stride, components, value) {
    ...
    for (let i = 0; i < l; i++) {
      for (let k = 0; k < components; k++) {
        dataView[setter](offset + i * stride + 4 * k, value[components * i + k], true);
      }
    }
  }
  ...
  function mergeTexturesFromMaterials(materials, textureNames) {
    ...
    for (const name of textureNames) {
      textureMap.indices[name] = texturesFromMaterials(materials, name, textureMap.textures);
    }
  ...
  function makeMaterialBuffer(gl, materials) {
    const maps = getTexturesFromMaterials(materials, ["map", "normalMap", "emissiveMap"]);
    const pbrMap = mergeTexturesFromMaterials(materials, ["roughnessMap", "metalnessMap"]);
    const pbrSgMap = mergeTexturesFromMaterials(materials, ["specularMap", "glossinessMap"]);
    const textures = {};
    const bufferData = {};
    ...
    bufferData.workflow = materials.map(m => "Metalness" === m.workflow ? 0 : 1);
    ...
    if (maps.map.textures.length > 0) {
      const {
        relativeSizes,
        texture
      } = makeTextureArray(gl, maps.map.textures, true, 4);
    ...
    if (maps.normalMap.textures.length > 0) {
      const {
        relativeSizes,
        texture
      } = makeTextureArray(gl, maps.normalMap.textures, false);
    ...
    if (pbrMap.textures.length > 0) {
      const {
        relativeSizes,
        texture
      } = makeTextureArray(gl, pbrMap.textures, false);
    ...
    if (pbrSgMap.textures.length > 0) {
      const {
        relativeSizes,
        texture
      } = makeTextureArray(gl, pbrSgMap.textures, false, 4);
    ...
    if (maps.emissiveMap.textures.length > 0) {
      const {
        relativeSizes,
        texture
      } = makeTextureArray(gl, maps.emissiveMap.textures, true);
  ...
  function makeTextureArray(gl, textures, gammaCorrection = false, channels = 3) {
    ...
    const {
      maxSize,
      relativeSizes
    } = maxImageSize(images); // create GL Array Texture from individual textures
  ...
  function uploadToUniformBuffer(gl, program, bufferData) {
    ...
    materialBuffer.set("Materials.alphaSpecularTintSheenSheenTint[0]", interleave({
      data: bufferData.alpha,
      channels: 1
    }, {
      data: bufferData.specularTint,
      channels: 1
    }, {
      data: bufferData.sheen,
      channels: 1
    }, {
      data: bufferData.sheenTint,
      channels: 1
    }));
    materialBuffer.set("Materials.clearcoaRoughnessSubfaceTransmission[0]", interleave({
      data: bufferData.clearcoat,
      channels: 1
    }, {
      data: bufferData.clearcoatRoughness,
      channels: 1
    }, {
      data: bufferData.subsurface,
      channels: 1
    }, {
      data: bufferData.transmission,
      channels: 1
    }));
    materialBuffer.set("Materials.iorAtDistanceAnisotropicWorkflow[0]", interleave({
      data: bufferData.ior,
      channels: 1
    }, {
      data: bufferData.atDistance,
      channels: 1
    }, {
      data: bufferData.anisotropic,
      channels: 1
    }, {
      data: bufferData.workflow,
      channels: 1
    }));
    materialBuffer.set("Materials.specularColorGlossiness[0]", interleave({
      data: [].concat(...bufferData.specularColor.map(d => d.toArray())),
      channels: 3
    }, {
      data: bufferData.glossiness,
      channels: 1
    }));
    materialBuffer.set("Materials.extinction[0]", interleave({
    ...
    materialBuffer.set("Materials.emissiveSpecularGlossinessMapIndex[0]", interleave({
      data: bufferData.emissiveMapIndex,
      channels: 1
    }, {
      data: bufferData.specularMapIndex,
      channels: 1
    }, {
      data: bufferData.glossinessMapIndex,
      channels: 1
    }, {
      data: bufferData.emissiveMapIndex,
      channels: 1
  ...
  async function makeRenderingPipeline({
    ...
    const maxReprojectedSamples = 20; // how many samples to render with uniform noise before switching to stratified noise

    const numUniformSamples = 4; // used to sample only a portion of the scene to the HDR Buffer to prevent the GPU from locking up from excessive computation
    ...
    const gBufferPass = makeGBufferPass(gl, {
      materialBuffer,
      mergedMesh
    });
    ...
    const fxaaPass = makeFXAAPass(gl, {
      fullscreenQuad
    });
    ...
    const svgfPass = makeSVGFPass(gl, {
      fullscreenQuad,
      toneMapping
    });
    ...
    let ready = false; // how many partitions of stratified noise should be created
    ...
    function spatialDenoiseToScreen(light) {
      let lightTexture = toneMapPass.draw({
        light
      }, true);
      fxaaPass.draw({
        light: lightTexture.color[0]
      });
    }
    ...
    function denoiseToneMapToScreen() {
      ...
      if (enableSpatialDenoise) if (enableTemporalDenoise) {
        spatialDenoiseToScreen(svgfPass.draw({
    ...
    function getDenoiseFactors() {
      return Object.assign(svgfPass.getDenoiseFactors(), reprojectPass.getDenoiseFactors());
    }
  ...
  const glRequiredExtensions = ["EXT_color_buffer_float", // enables rendering to float buffers
  "EXT_float_blend"];
  const glOptionalExtensions = ["OES_texture_float_linear" // enables gl.LINEAR texture filtering for float textures,
  ];
  class RayTracingRenderer {
    constructor(params = {}) {
      ...
      this._toneMapping = THREE__namespace.LinearToneMapping;
      ...
      this.size = new THREE__namespace.Vector2(this.canvas.width, this.canvas.height);
      this.pixelRatio = 1;
      this.pipeline = null;
    ...
    async buildScene(scene, camera) {
      ...
      if (typeof (loadingCallback == null ? void 0 : loadingCallback.onComplete) == "function") {
    ...
    set bounces(bounces) {
      ...
      (_this$pipeline = this.pipeline) == null ? void 0 : _this$pipeline.updateBounces(bounces);
    }
    ...
    set envMapIntensity(envMapIntensity) {
      ...
      (_this$pipeline2 = this.pipeline) == null ? void 0 : _this$pipeline2.setEnvMapIntensity(envMapIntensity);
    }
    ...
    set toneMapping(toneMapping) {
      ...
      (_this$pipeline3 = this.pipeline) == null ? void 0 : _this$pipeline3.setToneMapping(toneMapping);
    }
    ...
    set enviromentVisible(enviromentVisible) {
      ...
      (_this$pipeline4 = this.pipeline) == null ? void 0 : _this$pipeline4.setEnviromentVisible(enviromentVisible);
    }
    ...
    set movingDownsampling(movingDownsampling) {
      ...
      (_this$pipeline5 = this.pipeline) == null ? void 0 : _this$pipeline5.setMovingDownsampling(movingDownsampling);
    }
    ...
    set enableDenoise(enableDenoise) {
      ...
      (_this$pipeline6 = this.pipeline) == null ? void 0 : _this$pipeline6.setDenoiseStatus(enableDenoise);
    }
    ...
    set enableTemporalDenoise(enableTemporalDenoise) {
      ...
      (_this$pipeline7 = this.pipeline) == null ? void 0 : _this$pipeline7.setTemporalDenoiseStatus(enableTemporalDenoise);
    }
    ...
    set enableSpatialDenoise(enableSpatialDenoise) {
      ...
      (_this$pipeline8 = this.pipeline) == null ? void 0 : _this$pipeline8.setSpatialDenoiseStatus(enableSpatialDenoise);
    }
    ...
    set fullSampleCallback(fullSampleCallback) {
      if (typeof fullSampleCallback == "function") {
        ...
        (_this$pipeline9 = this.pipeline) == null ? void 0 : _this$pipeline9.setfullSampleCallbackCallBack(fullSampleCallback);
      }
    }
    ...
    updateEnvLight() {
      var _this$pipeline10;

      (_this$pipeline10 = this.pipeline) == null ? void 0 : _this$pipeline10.updateEnvLight();
    }

    updateMeshLight() {
      var _this$pipeline11;

      (_this$pipeline11 = this.pipeline) == null ? void 0 : _this$pipeline11.updateMeshLight();
    }

    setDenoiseColorBlendFactor(value) {
      var _this$pipeline12;

      (_this$pipeline12 = this.pipeline) == null ? void 0 : _this$pipeline12.setDenoiseColorBlendFactor(value);
    }

    setDenoiseMomentBlendFactor(value) {
      var _this$pipeline13;

      (_this$pipeline13 = this.pipeline) == null ? void 0 : _this$pipeline13.setDenoiseMomentBlendFactor(value);
    }

    setDenoiseColorFactor(value) {
      var _this$pipeline14;

      (_this$pipeline14 = this.pipeline) == null ? void 0 : _this$pipeline14.setDenoiseColorFactor(value);
    }

    setDenoiseNormalFactor(value) {
      var _this$pipeline15;

      (_this$pipeline15 = this.pipeline) == null ? void 0 : _this$pipeline15.setDenoiseNormalFactor(value);
    }

    setDenoisePositionFactor(value) {
      var _this$pipeline16;

      (_this$pipeline16 = this.pipeline) == null ? void 0 : _this$pipeline16.setDenoisePositionFactor(value);
    }

    setDemodulateAlbedo(value) {
      var _this$pipeline17;

      (_this$pipeline17 = this.pipeline) == null ? void 0 : _this$pipeline17.setDemodulateAlbedo(value);
      this.needsUpdate = true;
    }

    getDenoiseFactors() {
      var _this$pipeline18;

      return (_this$pipeline18 = this.pipeline) == null ? void 0 : _this$pipeline18.getDenoiseFactors();
    }
    ...
    getTotalSamples() {
      var _this$pipeline19;

      return (_this$pipeline19 = this.pipeline) == null ? void 0 : _this$pipeline19.getTotalSamplesRendered();
    }
  ...
  class DirectionalLight extends THREE$1.Light {
  ...
  class RectAreaLight extends THREE$1.Light {
  ...
  class SphereAreaLight extends THREE$1.Light {
  ...
  class PointLight extends THREE$1.Light {
    constructor(color, intensity) {
      super(color, intensity);
  ...
  class QuadLight extends THREE$1.Light {
